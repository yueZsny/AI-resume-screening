...
export function AiScreeningSettingsModal({
  ...
  return (
    ...
    >
      <div
        className="absolute inset-0 bg-[var(--app-overlay)] backdrop-blur-sm transition-opacity"
        aria-hidden
      />

      <div className="relative flex max-h-[min(92vh,880px)] w-full max-w-2xl flex-col overflow-hidden rounded-t-2xl border border-[var(--app-border)] bg-[var(--app-surface)] shadow-[var(--app-shadow)] sm:rounded-2xl">
        ...
        <div className="min-h-0 flex-1 overflow-y-auto p-4 sm:p-5">
          <div className="flex flex-col gap-5">
            ...
            <div>
              ...
              {loadingAiConfigs ? (
                <div className="flex h-12 items-center justify-center rounded-xl border border-dashed border-[var(--app-border)] bg-[var(--app-surface-raised)]/60">
                  <Loader2
                    className="h-5 w-5 animate-spin text-[var(--app-text-muted)]"
                    aria-hidden
                  />
                  <span className="sr-only">加载配置中</span>
                </div>
              ) : aiConfigs.length === 0 ? (
                <div className="rounded-xl border border-dashed border-[var(--app-warning)]/35 bg-[var(--app-warning-soft)] px-4 py-3 text-sm text-[var(--app-warning)]">
                ...
                <select
                  ...
                  className="h-12 w-full rounded-xl border border-[var(--app-border)] bg-[var(--app-surface)] px-3.5 text-sm text-[var(--app-text-primary)] focus:border-[var(--app-primary)] focus:outline-none focus:ring-2 focus:ring-[var(--app-ring)]"
                >
                  {aiConfigs
        ...
        <div className="flex shrink-0 flex-col-reverse gap-2 border-t border-[var(--app-border-subtle)] bg-[var(--app-surface-raised)]/90 px-4 py-3.5 sm:flex-row sm:justify-end sm:gap-3 sm:px-5">
          <button
            type="button"
            onClick={onClose}
          ...
          <button
            ...
            className="inline-flex items-center justify-center gap-2 rounded-xl bg-[var(--app-primary)] px-4 py-2.5 text-sm font-semibold text-white shadow-md shadow-[var(--app-primary)]/25 transition-colors hover:bg-[var(--app-primary-hover)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--app-ring-strong)] focus-visible:ring-offset-2 focus-visible:ring-offset-[var(--app-surface)] disabled:cursor-not-allowed disabled:opacity-50"
          >
            {screeningAll ? (
