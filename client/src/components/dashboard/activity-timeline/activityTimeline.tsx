  ...
  return (
    <div className="relative flex gap-0">
      <div className="flex w-8 shrink-0 flex-col items-center pt-1">
        ...
        {!isLast ? (
          <span
            className="mt-1 min-h-5 w-px flex-1 bg-zinc-200"
            aria-hidden
          />
        ) : null}
      </div>
      <button
        type="button"
        onClick={goResume}
        className="group min-w-0 flex-1 rounded-xl py-1.5 pl-3 pr-2 text-left transition-colors hover:bg-zinc-50/90 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-sky-500/20"
      >
        <p className="text-[13px] leading-snug text-zinc-800">
          <span className="font-medium text-zinc-900">{headline}</span>
        </p>
        <p className="mt-1 flex flex-wrap items-center gap-x-2 gap-y-0.5 text-[11px] text-zinc-500">
          <span className="rounded-md bg-zinc-100/90 px-1.5 py-0.5 font-medium text-zinc-600">
            {kindLabel}
          </span>
        ...
        {showDescBlock ? (
          <p className="mt-2 max-h-24 overflow-y-auto rounded-lg bg-zinc-50/90 px-2.5 py-2 text-left text-[12px] leading-relaxed text-zinc-600 ring-1 ring-zinc-100">
            {desc}
          </p>
        ) : null}
      </button>
